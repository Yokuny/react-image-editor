...
const ToolbarText: React.FC = () => {
  ...
  return useObserver(() => (
    <div className="toolbar__content">
      <button onClick={() => textStore.addText()} className="toolbar__action-btn">
        Add Text
      </button>
      {objectManagerStore.selectedObject ? (
        <>
          ...
          />
          <ColorPicker title="Colors" currentColorCode={textStore.fontColorCode} callback={(rgbCode) => textStore.setFontColor(rgbCode)} />
          <ToggleButton title="Background" checked={!textStore.isBgTransparent} callback={() => textStore.toggleBackground()} />
          {!textStore.isBgTransparent && <ColorPicker currentColorCode={textStore.bgColorCode} callback={(rgbCode) => textStore.setBackgroundColor(rgbCode)} />}
          <button className="toolbar__action-btn" onClick={() => objectManagerStore.deleteSelectedObject()}>
            Remove
          </button>
        </>
      ) : null}
